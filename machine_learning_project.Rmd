...
```{r, warning = FALSE}
modrf <- randomForest(classe ~ ., data = train_set, ntree = 50)
```

Next we predict using the model we've created on the validation set.

```{r}
predictrf <- predict(modrf, cross_set, type = "class")
cm_rf <- confusionMatrix(predictrf, cross_set$classe)
cm_rf
```

The random forest model gave a surprising 99.6% accuracy. There might be a danger of overfitting here due to the high accuracy. In order to test that we will use the test set provided and for further questions in the assignment.

```{r}
predict_test <- predict(modrf, test, type = "class")
predict_test
```